...
def add_userdata(username,password):
    ...
    cur.execute('INSERT INTO user_all(username,password) VALUES(?,?)',(username,hashed_password))
    conn.commit()
    
# Login function
def login_user(username,password):
    cur.execute('SELECT * FROM user_all WHERE username=? AND password=?',(username,password))
    data=cur.fetchall()
    return data
                    ...
                    if task == "Data Overview":
                        ...
                        if uploaded_file is not None:
                           ...
                           # my_report = dtale.show()
                            ...
                            tab31, tab32,tab11, tab12, tab13, tab14, tab15 = st.tabs(["Check for Duplicates","Handling Missing Data","Change Timestamp", "Drop Column", "Rename Column", "Change Datatypes","Save Dataset"])
                            ...
                            with tab32:
                                ...
                                dr = st.multiselect("Select columns to drop", df.columns, key="vdf")
                                for col in dr:
                            ...
                            with tab11:
                                #   Time Stamp
                                st.header("Change Timestamp")
                                tab21, tab22, tab23, tab24 = st.tabs(["Extract Date", "Encode Day Of The Week", "Encode Months","Encode Seasons"])
                            ...
                            with tab12:
                                ...
                                if st.button("Show Updated DataFrame"):
                            ...
                            with tab13:
                                ...
                                if st.button("Show Updated DataFrame", key="rename_col"):
                                        st.header("Updated DataFrame")
                                        st.write(df)

                            with tab14:
                                # Dealing with datatypes
                                st.header("Change Datatypes")
