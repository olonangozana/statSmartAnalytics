      ...
      if result :
          ...
          if task == "Data Overview":
                 ...
                 if uploaded_file is not None:
                    ...
                    selected_tab = st.selectbox("Select a section to explore", ["Dimensions", "Column Names", "Unique Values", "Missing Values", "Preview Dataset", "Duplicate Rows", "Data Types", "Dataset Summary"])

                    if selected_tab == "Dimensions":
                        st.header("Dimensions")
                        st.write(df.shape)

                    elif selected_tab == "Column Names":
                        st.header("Column Names")
                        st.write(df.columns)

                    elif selected_tab == "Unique Values":
                        st.header("DataFrame Unique Values")
                        st.write(df.nunique())
                    ...
                    elif selected_tab == "Preview Dataset":
                    ...
                    elif selected_tab == "Dataset Summary":
              ...
              selected_action = st.selectbox("Select an action", ["Check for Duplicates", "Handle Missing Data", "Clean Data Types", "Change Timestamp", "Drop Columns", "Rename Column", "Save Dataset"])

             # Check for duplicates
              if selected_action == "Check for Duplicates":
             ...
             # Handle missing data
              elif selected_action == "Handle Missing Data":
             ...
             # Clean data types
              elif selected_action == "Clean Data Types":
             ...
             # Change Timestamp
              elif selected_action == "Change Timestamp":
                 ...
                 # Drop Columns
              elif selected_action == "Drop Columns":
                    ...
                    # Rename Column
              elif selected_action == "Rename Column":
                   ...
                   # Save Dataset
              elif selected_action == "Save Dataset":
              ...
              #initializing an instance of Pandas AI with openAI environment
              pandas_ai = PandasAI(llm, verbose=True, conversational=False)
              PROMPT = st.chat_input("Want more insight? Talk to me", key="analyse this data")    
              # using pandasAI instance to process text prompt and dataset
              response = pandas_ai(df, PROMPT)
              # printing the response
              st.write(response)
              ...
              if filename is not None:
                  ...
                  if st.checkbox("Show Infinites"):
                      st.wite(df.isin([np.inf, -np.inf]).sum())
